...
> Alarm message supports variable `${msg}`, which can be used in `URL`, `Headers`, and `Body`; variable `${alertTime}` can be used in `Body` to represent the alert created time.
